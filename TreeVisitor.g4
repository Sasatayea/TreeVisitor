...
method:Ws* ((P|'public') Ws* 'static'?) Ws* ('int'|'boolean'|'Tree'|'Visitor') Ws* Id Ws*'('Ws*
        (('int'|'boolean'|'Tree'|'Visitor') Ws* Id Ws*)?(','Ws*('int'|'boolean'|'Tree'|'Visitor') Ws* Id Ws*)*')'Ws*
       '{' Ws* (if|nestedif|while|var|print)* WS*
        'return' Ws* (Id|Boolean|Integer) Ws* Scol Ws* '}'Ws*;
...
var_child: Ws* Id Ws* Eq Ws* Id Ws* (Id|'Tree'|'Visitor') Ws*'('Ws*')' Ws* Scol Ws* ;
var: varSignif|varInt|varFloat|varBool|varString|varTree|varId|var_op|var_dot|var_child;

ifCond: Ws*Id Ws*| Ws* Id Ws* (Condoperation) Ws*Id Ws*|Ws* '!'Ws*'('Ws*Id Ws* Condoperation Ws*Id Ws* ')'|Ws* Id'.'Id'('Ws*')'Ws* |Ws*Id Ws* ;
...
//tokens
Type:('int'|'double'|'string'|'char'|'long'|'float'|'boolean'|'Tree'|'Visitor');
Integer:('0'..'9')+;
Float: ('0'..'9')*('.')('0'..'9');
Boolean:('true'|'false');
